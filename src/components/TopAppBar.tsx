...
export const TopAppBar: React.FC = () => {
  ...
  const showBackButton =
    pathname !== "/home" && pathname !== "/sign-in";
  ...
  const handleStatusClick = () => {
    if (isOnline) {
      speak(
        "You are currently online. Cloud AI features and real-time syncing are enabled.",
        true
      );
    } else {
      speak(
        "You are currently offline. Core features like Speech, Speak For Me, Live Captions, and Local Translations are running offline.",
        true
      );
    }
  };
  ...
  const handleInstallClick = async () => {
    speak(
      "Opening application installation prompt",
      true
    );
  ...
  return (
    <header className="w-full top-0 sticky shadow-sm z-40 bg-surface border-b border-outline-variant transition-colors">
      <div className="w-full max-w-7xl px-2 sm:px-4 md:px-8 mx-auto flex items-center justify-between h-16">
        ...
        {/* Left side: Menu + Back Button + Logo + Status */}
        <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
          ...
          {showBackButton && (
            ...
            >
              <span className="material-symbols-outlined font-semibold text-xl sm:text-2xl">
                arrow_back
              </span>
            </button>
          )}
          ...
          <button
            ...
            aria-label={
              isOnline
                ...
                : "Offline status: Local mode active"
            }
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          {!isInstalled && (
            <button
              ...
              title="Install App"
              className="flex items-center justify-center w-9 h-9 sm:w-auto sm:h-9 sm:px-3 rounded-xl bg-primary/10 hover:bg-primary/20 text-primary active:scale-95 transition-all cursor-pointer"
            >
              <span className="material-symbols-outlined text-xl">
                install_mobile
              </span>

              <span className="hidden sm:inline ml-1.5 font-bold text-sm">
                Install App
              </span>
            </button>
          )}
