    ...
    try {
      ...
      if (base64) {
        ...
        if (result.objects && result.objects.length > 0) {
          const mapped: DetectedObject[] = result.objects.map((obj) => ({
            label: obj.name,
            x: `${(obj.boundingBox?.left ?? 0) * 100}%`,
            y: `${(obj.boundingBox?.top ?? 0) * 100}%`,
            w: `${Math.max(8, (obj.boundingBox?.width ?? 0.2) * 100)}%`,
            h: `${Math.max(8, (obj.boundingBox?.height ?? 0.2) * 100)}%`,
        ...
        if (result.hazards && result.hazards.length > 0) {
          ...
          addToast("Scene analyzed with Cloud Vision", "success");
      ...
      const message = "Live scene analysis is unavailable. Check your camera permission and Cloud Vision key.";
      setDescription(message);
      setSceneSource("unavailable");
      speak(message, true);
      addToast("Scene analysis unavailable", "error");
    } finally {
              ...
              <div className="flex items-center gap-2">
                ...
                {sceneSource && (
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${sceneSource === "google-cloud-vision" ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600"}`}>
                    {sceneSource === "google-cloud-vision" ? "Live Vision" : "Unavailable"}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {detectedObjects.map((obj, i) => (
                    <span key={i} className="bg-primary/10 text-primary font-bold text-sm px-3 py-1 rounded-full border border-primary/20">
                      {obj.label}
                      {obj.position && <span className="ml-1 opacity-70">({obj.position})</span>}
                      {obj.confidence && (
